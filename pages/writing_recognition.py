...
def get_ocr():
    ...
    for kwargs in [
        dict(lang="korean", use_angle_cls=True, device="gpu"),  # ✅ GPU 사용
        dict(lang="korean", device="gpu"),
        dict(device="gpu"),  # 최후의 수단, GPU 강제
    ]:
        try:
            return PaddleOCR(**kwargs)
        except TypeError:
            continue
    st.error("❌ PaddleOCR 초기화 실패: 버전/환경 호환 문제가 있습니다. (CUDA 지원 여부 확인 필요)")
    st.stop()
